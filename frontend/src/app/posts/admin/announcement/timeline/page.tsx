...
export default function AnnouncementTimeline(){
    ...
    const handleMouseMove = (e: React.MouseEvent, item: Announcement) => {
        setTooltipData({
            visible: true,
            x: e.clientX,
            y: e.clientY,
                            ...
                            {isExpanded && announcements?.map((item) => {
                                ...
                                return (
                                    <div key={`timeline-${item.id}`} className="h-12 flex items-center relative w-full border-b border-transparent hover:bg-gray-50/50">
                                        ...
                                        >
                                            <span className="text-white text-xs font-medium truncate w-full block select-none pointer-events-none"> {/* เพิ่ม pointer-events-none เพื่อกันเมาส์กระพริบ */}
                                        ...
                                        {tooltipData.visible && tooltipData.item && (
                                            <div 
                                                className="fixed z-[9999] bg-gray-800 text-white text-xs rounded shadow-xl border border-gray-700 pointer-events-none overflow-hidden min-w-[200px]"
                                                style={{ 
                                            ...
                                            >
                                                {/* Header */}
                                                <div className="px-3 py-2 border-b border-gray-600 font-bold text-sm break-words bg-gray-900/50">
                                                ...
                                                <div className="px-3 py-2 bg-gray-800">
                                                    <div className="grid grid-cols-[40px_1fr] gap-y-1">
                                                        ...
                                                        <span className="text-gray-200">{formatDateTime(tooltipData.item.start_at)}</span>
                                                        
                                                        <span className="text-gray-400 font-medium">End:</span>
                                                        <span className="text-gray-200">{formatDateTime(tooltipData.item.end_at)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
