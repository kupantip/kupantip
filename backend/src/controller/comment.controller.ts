...
async function isPostExist(post_id: string): Promise<boolean> {
	...
	const result = await pool
		...
		.query(
			`SELECT 1 FROM [dbo].[post] WHERE id = @post_id AND deleted_at IS NULL`
		);
	...
	try {
		...
		if (parent_id) {
			...
			const result = await pool
				...
				.query(
					`SELECT 1 FROM [dbo].[comment] WHERE id = @parent_id AND deleted_at IS NULL`
				);

			// true = found, false = not found
			if (result.recordset.length <= 0) {
				res.status(404).json({
					message: 'Parent comment_id not found',
				});
				return;
			}
		}
